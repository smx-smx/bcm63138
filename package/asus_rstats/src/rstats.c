		...
		for (i = 15; i > 0; --i) {
			if (!wait_action_idle(10)) {
				...
				if (eval("cp", hgz, tmp) == 0) {
					...
					rstats_dbg("rename %s %s\n", tmp, save_path);
					if (rename(tmp, save_path) == 0) {
						rstats_dbg("rename ok\n");
						break;
					}
				}
			}
...
static int decomp(const char *fname, void *buffer, int size, int max)
{
	...
	if ((size * max) != file_size) {
		rstats_dbg("filesize mismatch! (%ld/%ld)\n", (size * max), file_size);
		goto exit_decomp;
	}
...
static void load(int new)
{
	...
	//rstats_dbg("uptime = %dm, save_utime = %dm\n", current_uptime / 60, save_utime / 60);

	//

	sprintf(hgz, "%s.gz", speed_fn);
	speed_count = decomp(hgz, speed, sizeof(speed[0]), MAX_SPEED_IF);
	rstats_dbg("speed_count = %d\n", speed_count);
	...
	rstats_dbg("read source=%s save_path=%s\n", sp, save_path);
	if ((strcmp(sp, save_path) == 0) && (load_history(hgz))) {
		rstats_dbg("using local file\n");
		return;
	}
...
static void save_speedjs(long next)
{
	...
	fprintf(f, "%s_next: %ld};\n", speed_count ? "},\n" : "", ((next >= 1) ? next : 1));
	fclose(f);
	rstats_dbg("write /var/tmp/rstats-speed.js end\n");
...
rstats_dbg("save_datajs:\n");
	...
	if (mode == DAILY) {
		...
		p = history.dailyp;
		max = MAX_NDAILY;
rstats_dbg("DAILY: p= %d\n", p);
	}
	else {
		data = history.monthly;
		p = history.monthlyp;
		max = MAX_NMONTHLY;
rstats_dbg("MONTHLY: p= %d\n", p);
	}
	kn = 0;
	for (k = max; k > 0; --k) {
		p = (p + 1) % max;
		...
		++kn;
rstats_dbg("%d:: [0x%lx,0x%llx,0x%llx]\n", p, 
	(unsigned long)data[p].xtime, data[p].counter[0] / K, data[p].counter[1] / K);
	}
	fprintf(f, "];\n");
}
...
static void save_histjs(void)
{
	...
	if ((f = fopen("/var/tmp/rstats-history.js", "w")) != NULL) {
		...
		fclose(f);
		//rstats_dbg("write /var/tmp/rstats-history.js end\n");
...
static void calc(void)
{
	...
	for (j = 0; j < MAX_IF_NUM; j++) 
	{
		...
		pclose(f);
		rstats_dbg(">>> calc_5g[wifi%d]: %s, %llu, %llu <<<\n", j, ifname_desc, counter[0], counter[1]);
	...
	while (fgets(buf, sizeof(buf), f)) {
		...
		*p = 0;
		...
		if ((strcmp(ifname, "lo") == 0) || (find_word(exclude, ifname))){
			rstats_dbg("%s: [%s] is lo or exclude\n", __func__, ifname);
			continue;
		}

		// <rx bytes, packets, errors, dropped, fifo errors, frame errors, compressed, multicast><tx ...>
		if (sscanf(p + 1, "%llu%*u%*u%*u%*u%*u%*u%*u%llu", &counter[0], &counter[1]) != 2){
			//rstats_dbg("%s can't find counter[0] and counter[1]\n", ifname);
			continue;
		}
	...
	for (t = 0, tmp = tmp_speed; t < ARRAY_SIZE(tmp_speed); ++t, ++tmp) {
		...
		if (sp->sync) {
			...
			n = (tick / INTERVAL);
			if (n < 1) {
				rstats_dbg("%s is a little early... %d < %d\n", tmp->desc, tick, INTERVAL);
				continue;
			}
	
			sp->utime += (n * INTERVAL);
			rstats_dbg("%s n=%d tick=%ld, sp->utime=%ld\n", sp->ifname, n, tick, sp->utime);
	...
	for (i = 0; i < speed_count; ++i) {
		...
		if (((current_uptime - sp->utime) > (10 * SMIN)) || (find_word(exclude, sp->ifname))) {
			rstats_dbg("#%d removing. > time limit or excluded\n", i);
			--speed_count;
			memcpy(sp, sp + 1, (speed_count - i) * sizeof(speed[0]));
		}
		else {
			rstats_dbg("%s not found setting sync=1, #%d\n", sp->ifname, i);
			sp->sync = 1;
		}
	}
	...
	if (current_uptime >= save_utime) {
		...
		rstats_dbg("uptime = %ldm, save_utime = %ldm\n", (current_uptime / 60), (save_utime / 60));
	}
}
...
int main(int argc, char *argv[])
{
	...
	while (1) {
		...
		rstats_dbg("current_uptime=%ld, z=%ld\n", current_uptime, z);
		calc();
	...
	while (n-- > 0) {
		...
		if (act.pid > 0 && !process_exists(act.pid)) {
			if (!(r = unlink(ACTION_LOCK)) || errno == ENOENT) {
				rstats_dbg("Terminated process, pid %d %s, hold action lock %d !!!\n",
					act.pid, act.comm, act.action);
				return 1;
			}
			rstats_dbg("Remove " ACTION_LOCK " failed. errno %d (%s)\n", errno, strerror(errno));
		}
		sleep(1);
	}
	rstats_dbg("pid %d %s hold action lock %d !!!\n", act.pid, act.comm, act.action);
	return 0;
}
...
EXIT:
	...
	if (nvram_match("debug_logeval", "1")) {
		...
		rstats_dbg("_eval +%ld pid=%d ", uptime(), pid);
		for (n = 0; argv[n]; ++n) rstats_dbg("%s ", argv[n]);
		rstats_dbg("\n");
