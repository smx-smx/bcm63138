...
inline void freeEntry(struct usb_entry *free_node) {
...
/* 
 * For multi-function printer, it has multiple USB interfaces.
 * Some of them MAY has duplicated class.
 */
#define CHECK_DUPLICATED_CLASS(o,n) do { \
    if( (o->root_port_number == n->root_port_number) && \
        (o->idVendor == n->idVendor) && \
        (o->idProduct == n->idProduct) && \
        (strcmp(o->Manufacturer, n->Manufacturer)==0) && \
        (strcmp(o->Product, n->Product)==0) && \
        (strcmp(o->SerialNumber, n->SerialNumber)==0) && \
        (o->DeviceClass == n->DeviceClass)){ \
        freeEntry(n); \
        return; \
    } \
} while(0);
...
/*
	...
	uint8_t DeviceClass;
 */
void append_usb_entry(struct usb_entry *newEntry) 
{
	...
	if(head==NULL) {
		...
		tail = head;
        CHECK_DUPLICATED_CLASS(tail, newEntry);
	...
	while ((dev = devs[i++]) != NULL) {
		...
		
		{
			...
			if (desc.iSerialNumber > 0)
			{
				...
				/* Adam 20170721, Remove space after SerialNumber. */
				int i = 0, j = 0;
				while( strSerialNumber[j]!='\0' ){
					if(strSerialNumber[j] != ' ' || strSerialNumber[j+1] != ' '){
						strSerialNumber[i]=strSerialNumber[j];
						i++;
						j++;
					}
					else{
						strSerialNumber[i]=strSerialNumber[j+1];
						j++;
					}
				}
				if( strSerialNumber[i-1] == ' ' )
					strSerialNumber[i-1] = '\0';
				else
					strSerialNumber[i]='\0';
			}
