...
if [ -f $apps_local_space/bzip2_1.0.6-1_arm.ipk ];
then
	ipkg install $apps_local_space/bzip2_1.0.6-1_arm.ipk
	if [ -f /opt/lib/libbz2.so.1.0.6 ];
	then
		cp -f /opt/lib/libbz2.so.1.0.6 $APPS_INSTALL_PATH/lib/
	fi
fi

if [ "$1" == "downloadmaster" ] && [ -f $apps_local_space/apps.patch ];
then
	cd /opt; patch -p1 < $apps_local_space/apps.patch
	cd $PWD
fi
...
## Adam 20160707, update firewall rules.
mngcli action ui_firewall_mod=app_mod
