...
processInternal(const char *sm_bin, dstrbuf *msgcon)
{
	...
	if (!open_sendmail) {
		...
		//Code Sung overwrite 20170327
		system("echo Processmail.c : ARC_PROJ_ASUS_FEEDBACK_STATE = 2 > /dev/console");
		system("mng_cli set ARC_PROJ_ASUS_FEEDBACK_STATE=2");
		//Code Sung overwrite 20170327 end
		return ERROR;
	}
