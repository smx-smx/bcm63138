...
#define aip_dbg(fmt, args...) do { \
		FILE *fp = fopen("/tmp/tm_aiprotection.log", "a"); \
		if (fp) { \
			fprintf(fp, fmt, ## args); \
			fclose(fp); \
		} \
...
// signature check
#define APPDB           nvram_get_int("bwdpi_debug_path") ? "/data/TM/bwdpi.app.db" : "/tmp/bwdpi/bwdpi.app.db"
#define CATDB           nvram_get_int("bwdpi_debug_path") ? "/data/TM/bwdpi.cat.db" : "/tmp/bwdpi/bwdpi.cat.db"
#define RULEV           nvram_get_int("bwdpi_debug_path") ? "/data/TM/rule.version" : "/tmp/bwdpi/rule.version"
...
#define WRS_VP_LOG      "/data/wrs_vp.txt"
#define SIG_VER         "/proc/nk_policy"
#define DPI_VER         "/proc/ips_info"
#define WAN_TMP         nvram_get_int("bwdpi_debug_path") ? "/data/TM/dev_wan" : "/tmp/bwdpi/dev_wan"
